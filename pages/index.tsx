            ...
            <div className="flex items-center mb-10 justify-between xl:justify-center text-white">
              <div className="xl:w-11/12 md:w-3/4 w-full">
                ...
                <h3 className="text-2xl">
                  Also, apply as a <span className="font-bold text-4xl">Developer</span>
                </h3>
              </div>
          ...
          <div className={`${styles.popularSkillsBgImg} mt-4 rounded`}>
            <div className="grid grid-cols-3 sm:grid-cols-5 max-w-6xl m-auto items-center justify-center gap-x-4 md:gap-x-6 lg:gap-x-11 gap-y-10 py-10 px-4 sm:px-[15px] md:px-[30px] lg:px-[60px] capitalize">
              {dummyData.map((item, i) => {
                return (
                  <div key={i} className="bg-white flex flex-col w-full h-full rounded p-3 md:p-5">
                    <div className="pl-2">{item.name}</div>
                    <span className="w-9 h-10 sm:w-11 sm:h-12 ml-auto mt-6">{item.icon}</span>
                  </div>
                );
              })}
            </div>
          ...
          <div className="flex items-center justify-between rounded-lg max-w-6xl m-auto mt-[70px] px-3 mb-10 md:px-7 xl:px-0">
            <div className="text-[15px] w-11/12 sm:w-1/2">
              <h2 className="text-[30px] text-gray2 font-bold mb-5 lg:mb-10">
                Apply as a <span className="font-extrabold uppercase text-black">Developer</span>
              </h2>
              Lorem ipsum dolor sit amet, consectetur adipisicing elit. Repudiandae sit deserunt,
              laboriosam mollitia magnam vel, sint officia necessitatibus doloribus sunt amet quia
              velit doloremque, aliquid ipsam deleniti suscipit! Possimus, illo!
              <div className="font-semibold mt-5 lg:mt-10">
                Lorem ipsum, dolor sit amet consectetur adipisicing elit. Facere, tenetur vel!
                Beata.
              </div>
              <NextLink href="/apply">
                <button
...
function AllSkills() {
  ...
  const alphabetically = [...dummyData].sort((a: any, b: any) => (a.name > b.name ? 1 : -1));
